...
if ($design == 'Advisor') {
  ...
  $sql = "SELECT * FROM registration r INNER JOIN noncertod o ON r.regno like o.regno 
  WHERE (r.batch like '$batch') AND (r.dept like '$dept') 
...
function getDetails()
{
  ...
  if ($design == 'Advisor') {
    ...
    WHERE (r.batch like '$batch') AND (r.dept like '$dept') and (o.start <='$today' and o.end >='$today') AND 
    ...
    if ($data->num_rows == 0 && $data2->num_rows == 0) {
      ...
      while ($row = mysqli_fetch_array($data2)) {
        $detailsList .= '<div class="title">
            ...
            <div class="content">
            <p>LOCAL OD:- ' . $row['activity'] . ' ~ ' . $row['hrs'] . ' ~ ' . $row['reason'] . ' </p>
            </div>';
      }
      echo $detailsList;
    }
